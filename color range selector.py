...
class ColorRange:
    ...
    def __init__(self, whitelistColors, blacklistColors):
        self.colors = []
...
def change_hue(
    ...
    cr = ColorRange(whitelist_colors_hue, blacklist_colors_hue)
    cr.removeDuplicates()
    ...
    pixels = image.load()
    whitelistHueSum = 0
    for color in whitelist_colors_hue:
        whitelistHueSum += color
    avgWhitelistHue = whitelistHueSum/len(whitelist_colors_hue)
    hue_adjust = new_color_hsl[0] - avgWhitelistHue
    last_percent_done = 0
    ...
    for i in range(image.size[0]):  # for every col:
        ...
        for j in range(image.size[1]):  # For every row
            ...
            hue_offset_to_target = pixel_hsl[0] - cr.closest_mark(pixel_hsl[0])
...
change_hue(img_path, new_img_path,
           [(198, 192, 178), (248, 243, 1)], [(70, 60, 3), (43, 37, 2), (125, 103, 20)], blue, 0.04)
